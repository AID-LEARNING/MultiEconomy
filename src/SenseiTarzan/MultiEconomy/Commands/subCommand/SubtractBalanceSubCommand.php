...
class subtractBalanceSubCommand extends EcoBaseSubCommand
{
	...
	public function onRun(CommandSender $sender, string $aliasUsed, array $args) : void
	{
		...
		Await::g2c($this->economy->subtract($target, $amount),
			function (bool $online) use ($target, $amount, $economy, $sender) {
				$sender->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($sender, CustomKnownTranslationFactory::subtract_economy_sender($target, $economy, $amount)));
				if (!$online) return;
				$target->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($target, CustomKnownTranslationFactory::subtract_economy_receiver($economy, $amount)));
			}, [
