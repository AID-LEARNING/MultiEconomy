...
class addBalanceSubCommand extends EcoBaseSubCommand
{
	...
	public function onRun(CommandSender $sender, string $aliasUsed, array $args) : void
	{
		...
		Await::g2c($this->economy->add($player, $amount),
			function (bool $online) use ($player, $amount, $economy, $sender) {
				$sender->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($sender, CustomKnownTranslationFactory::add_economy_sender($player, $economy, $amount)));
				if (!$online) return;
				$player->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($player, CustomKnownTranslationFactory::add_economy_receiver($economy, $amount)));
			}, [
